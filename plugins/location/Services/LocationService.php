...
class LocationService
{
    ...
    public function __construct(LocationManager $manager)
    {
        ...
        $this->initializeCacheSettings();
    }

    /**
     * 初始化缓存设置
     */
    protected function initializeCacheSettings(): void
    {
        try {
            // 从插件配置获取缓存设置
            $pluginConfigManager = app(\App\Modules\PluginSystem\Services\PluginConfigManagerService::class);
            $this->cacheEnabled = $pluginConfigManager->getPluginConfigValue('Location', 'LOCATION_CACHE_ENABLED', true);
            $cacheTtlMinutes = $pluginConfigManager->getPluginConfigValue('Location', 'LOCATION_CACHE_TTL', 1440);
            $this->cacheTtl = $cacheTtlMinutes * 60; // 转换为秒
        } catch (Exception $e) {
            // 回退到默认设置
            $this->cacheEnabled = true;
            $this->cacheTtl = 86400; // 24小时
        }
    }
